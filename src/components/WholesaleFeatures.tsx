  ...
  return <section className="py-20 bg-background">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          {features.map((feature, index) => <Card key={index} className="bg-gradient-card shadow-card hover:shadow-hero transition-all duration-300 border-border relative overflow-hidden">
              {feature.badge && <Badge className="absolute top-4 right-4 bg-accent text-accent-foreground">
                  {feature.badge}
                </Badge>}
              <CardHeader>
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-2 mb-6">
                  {feature.benefits.map((benefit, idx) => <div key={idx} className="text-sm text-muted-foreground flex items-center">
